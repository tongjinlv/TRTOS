...
R:  GPRS_OFF;
    ...
    M26_RESET_OFF;
    Tos_TaskDelay(1000);
		if(IO_Get(8))DeBug("开机成功",Infor_Warning);
		else {DeBug("模块开机失败",Infor_Warning);GPIO_Set_PWR(0);if(UIShowValue.SaveValue.SleepMin)TOS_Sleep(UIShowValue.SaveValue.SleepMin);Tos_TaskDelay(11000);goto R;}
	  Tos_TaskDelay(4000);
...
u8 QuectelM26_SendCaiji(HistoryRecord *HR)
{
	uint8 Buf[500];
	uint8 c,c1;
	uint16 i=0;
	if(UIShowValue.SaveValue.WorkBit&WCB_MQTTEN)
	{
		...
		i+=GetStringByFmt((char *)&Buf[i],"{\"value\":%.2f},",UIShowValue.CH_Value[1]);
			...
			{
				GPIO_Set_Mode1();
				i+=GetStringByFmt((char *)&Buf[i],"{\"switcher\":%d},",GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_8));
			}
		i+=GetStringByFmt((char *)&Buf[i],"{\"value\":%.2f},",UIShowValue.GPS_Longitude);
		i+=GetStringByFmt((char *)&Buf[i],"{\"value\":%.2f},",UIShowValue.GPS_Latitude);
		i+=GetStringByFmt((char *)&Buf[i],"{\"value\":%.2f},",UIShowValue.CH_Value[2]);
		i+=GetStringByFmt((char *)&Buf[i],"{\"value\":%.2f}",(float)UIShowValue.RSSI);
		//i+=GetStringByFmt((char *)&Buf[i],"{\"timestamp\":\"20%02d-%02d-%02d %02d:%02d:%02d\"}",HR->DT.Year,HR->DT.Mon,HR->DT.Day,HR->DT.Hour,HR->DT.Min,HR->DT.Sec);
		i+=GetStringByFmt((char *)&Buf[i],"]}");
...
R:
	...
	UIShowValue.RSSI=99;
	sta=QuectelM26_Init();
	if(sta){DeBug("注册网络失败[%d] ",sta,Infor_Warning);if(UIShowValue.SaveValue.SleepMin){Task_RecordNow();TOS_Sleep(UIShowValue.SaveValue.SleepMin);} goto R;}
	else Task_AutoReg(0);
	sta=QuectelM26_InitSMS(1);
	if(sta)DeBug("初始化短信[%d]",sta,Infor_Warning);
		...
		if(CurMsg)
		{
			switch(CurMsg->Qmsg_Type)
			{
				case 0:
					...
					sta=QuectelM26_Connect();
					QuectelM26_Config();
					sta=QuectelM26_SendCaiji(TaskMsg);
					if(sta)
								{
									sta=QuectelM26_SendCaiji(TaskMsg);
									if(sta)
...
R:
					...
					if(sta)
								{
									...
									if(sta)DeBug("发送数据失败[%d]",sta,Infor_Warning);FailCount++;
											...
											DeBug("发送数据OK",Infor_Warning);
											FailCount=0;RFailCount=0;
											//TOS_Sleep(UIShowValue.SaveValue.SleepMin);
							  }
	...
	//	if(TimeSleep>10)if(UIShowValue.SaveValue.SleepMin)TOS_Sleep(UIShowValue.SaveValue.SleepMin);
		Tos_TaskDelay(1000);
		if(FailCount>0){Tos_TaskDelay(2000);QuectelM26_Connect();FailCount=0;QuectelM26_Config();RFailCount++;}
		if(RFailCount>3){Tos_TaskDelay(11000);goto R;}
