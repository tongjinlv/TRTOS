...
http://www.trtos.com/
**************************************************************************************/
#include <..\USER\Prj_AirCleaner\Ht1621_Driver.h>
...
{
	{GPIO_Speed_2MHz,GPIO_Mode_Out_PP,HT1621_GROUP,HT1621_CS|HT1621_WR|HT1621_DAT},//LCD端口初始化
	{0},
};
/*******************************************************************************
Func:LCD写入数据
...
void Ht1621Wr_Data(u8 Data, u8 cnt)
{
	...
	for (i = 0; i < cnt; i++)
	{
	 GPIO_ResetBits(HT1621_GROUP,HT1621_WR);  
	 Ht1621_Delay(3);
	 if (Data & 0x80)
	 {
			GPIO_SetBits(HT1621_GROUP,HT1621_DAT); 
	 }
	 else
	 {
			GPIO_ResetBits(HT1621_GROUP,HT1621_DAT);
	 }
	 Ht1621_Delay(3);
	 GPIO_SetBits(HT1621_GROUP,HT1621_WR); 
	 Data <<= 1;
	 Ht1621_Delay(1);
	}
}
...
{
	GPIO_ResetBits(HT1621_GROUP,HT1621_CS);  //CS = 0;  先拉底，准备传数据
	Ht1621_Delay(3);
  Ht1621Wr_Data(0x80, 4); //写入命令标志 "100+0",命令格式="100+0+8位的命令字节"
  Ht1621Wr_Data(Cmd, 8);  //写入命令数据
	GPIO_SetBits(HT1621_GROUP,HT1621_CS);   //不写数据就再次拉高
...
void Ht1621_WriteReg(u8 Addr, u8 Data)   //可以控制四个段
{
	...
	GPIO_SetBits(HT1621_GROUP,HT1621_CS);     //拉高，终止传数据 
}
/*******************************************************************************
Func:LCD 连续写寄存器
Date:2016-3-11
...
void Ht1621_WriteRegs(u8 SPaddr, u8* SpData, u8 Len)
{
	...
	GPIO_SetBits(HT1621_GROUP,HT1621_CS);  // //CS拉高，片选结束
}
/*******************************************************************************
Func:LCD 初始化
Date:2016-3-11
