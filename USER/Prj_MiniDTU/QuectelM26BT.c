...
http://www.trtos.com/
**************************************************************************************/
#include <..\USER\Prj_MiniDTU\QuectelM26.h>
...
u8 QuectelM26_Init(void)
{
		...
		GPIO_SetIndex(2,0);
	 	Tos_TaskDelay(3000);
	  if(GPRS_VDD)DeBug("开机成功",Infor_Warning);
		else
		{
			GPIO_SetIndex(2,1);
			Tos_TaskDelay(2000); 
...
u8 QuectelM26_Transmission()
{
	...
	R:
	if(c--<1)break;
	if(!GPRS_CommandAck("AT+QIMODE=1\r\n","OK",1000))goto R;
	if(!GPRS_CommandAck("AT+QIMUX=0\r\n","OK",1000))goto R;
	if(!GPRS_CommandAck("AT+QIREGAPP\r\n","OK",1000))goto R;
	Tos_TaskDelay(300);
	if(!GPRS_CommandAck("AT+QIDNSIP=1\r\n","OK",1000))goto R;
	Tos_TaskDelay(300);
	if(!GPRS_CommandAck("AT+QIACT\r\n","OK",1000))goto R;
	Tos_TaskDelay(300);
	if(!GPRS_CommandAck("AT+QITCFG=3,2,1400,1\r\n","OK",1000))goto R;
	Tos_TaskDelay(300);
	break;
	}
		StringCoppy("link.tlink.io",&UIShowValue.SaveValue.DomainName[0][0]);
	UIShowValue.SaveValue.ServerPort[0]=8647;
	FmtToBuf(&Buf[0],"AT+QIOPEN=\"TCP\",\"%s\",%d\r\n",&UIShowValue.SaveValue.DomainName[0][0],UIShowValue.SaveValue.ServerPort[0]);
	Tos_TaskDelay(300);
	//DeBug("链接到%s","AT+QIOPEN=\"TCP\",\"119.136.112.41\",9112\r\n",5);
	//USART_WriteLine(USART2,"AT+QIOPEN=\"TCP\",\"119.136.112.41\",9112\r\n");
	USART_WriteLine(USART2,"AT+QIOPEN=\"TCP\",\"link.tlink.io\",8647\r\n");
	Tos_TaskDelay(1000);
	DeBug("设置为透传模式",Infor_Warning);
	return 0;
}
...
u8 QuectelM26_SendHTTPJson_(uint8 *HR)
{
	uint8 Buf[400],Data[100];
	uint8 c,c1;
	uint16 i=0;
	if(UIShowValue.SaveValue.WorkBit&WCB_MQTTEN)
	{
	...
	//	i+=GetStringByFmt((char *)&Data[i],"20%02d-%02d-%02d %02d:%02d:%02d;",HR->DT.Year,HR->DT.Mon,HR->DT.Day,HR->DT.Hour,HR->DT.Min,HR->DT.Sec);
		i+=GetStringByFmt((char *)&Data[i],HR);
		i=0;
		UIShowValue.SaveValue.LoginHead[sizeof(UIShowValue.SaveValue.LoginHead)-1]=0;
		i+=GetStringByFmt((char *)&Buf[i],"POST /php/api.php?id=%s HTTP/1.1\n",UIShowValue.SaveValue.LoginHead);
		i+=GetStringByFmt((char *)&Buf[i],"Content-Type: application/x-www-form-urlencoded\n");
		...
		i+=GetStringByFmt((char *)&Buf[i],"Connection: Keep-Alive\r\n\r\n");
		i+=GetStringByFmt((char *)&Buf[i],"%s\r\n",&Data[0]);
		DeBug("发送数据:%s",&Data[0],Infor_Warning);
		USART_WriteLine(USART2,"AT+QISEND=%d\r\n",i);
		if(!GPRS_AckDelay(">",2000))return 1;
		USART_WriteDatas(USART2,&Buf[0],i);
		if(!GPRS_AckDelay("SEND OK",1000))return 2;//检查发送成功
...
u8 QuectelM26_SendHTTPJson(uint8 *HR)
{
	uint8 Buf[500],Data[300],Tdata[10];
	uint16 i=0,d;
	DeBug(HR,Infor_Warning);
    ...
    i+=GetStringByFmt((char *)&Data[i],"\"data\":[{");
		d=FindFloatFromString(HR,Tdata,0);
		Tdata[d]=0;
		i+=GetStringByFmt((char *)&Data[i],"\"deviceNo\":\"%s\",",Tdata);
		d=FindFloatFromString(HR,Tdata,1);
		Tdata[d]=0;
		i+=GetStringByFmt((char *)&Data[i],"\"temperature\":%s,",Tdata);
		d=FindFloatFromString(HR,Tdata,2);
		Tdata[d]=0;
		i+=GetStringByFmt((char *)&Data[i],"\"humidity\":%s",Tdata);
		i+=GetStringByFmt((char *)&Data[i],"}]}");
		i=0;
		DeBug(Data,Infor_Warning);
...
void Timer_BrokenPinP()
{
	uint8 Buf[200];
	StringCoppy("{df}dffdsafsaf}fdsadfsaddddddddddddddddddddddddddddddddd",Buf);
	Tos_TaskPutQmsg(Tos_GetTaskIDByName("GPRS"),0,200,Buf);
}
void Timer_Broken1()
{
...
void Task_M26HTTPJSON(void *Tags)
{
	uint8 sta,i,FailCount=0,RFailCount;
	uint8 SendBuf[200];
	Tos_ArrayMessage *CurMsg,Msg[3];
	uint8 *TaskMsg,TaskMsgBuf[3][200];	
	for(i=0;i<3;i++)Msg[i].Qmsg_Addr=&TaskMsgBuf[i];
	Tos_TaskCreateQmsg(&Msg[0],3);
R:
	RFailCount=0;
	...
	if(sta)DeBug("初始化短信[%d]",sta,Infor_Warning);
	PDU_GetCenterNumber();
	//Tos_CreateTimer(UIShowValue.SaveValue.SendCycle,0xffff,Timer_BrokenPinP);
	//Timer_BrokenPinP();
	Tos_TaskDelay(3000);
	while(1)
	{
		...
		if(CurMsg)
		{
			switch(CurMsg->Qmsg_Type)
			{
				case 0:
					...
					sta=QuectelM26_Connect();
					if(sta)DeBug("连接服务器[%d]",sta,Infor_Warning);
					sta=QuectelM26_SendHTTPJson(TaskMsg);
					if(sta)
								{
									DeBug("发送失败,重新发送[%d]",sta,Infor_Warning);
									sta=QuectelM26_SendHTTPJson(TaskMsg);
...
void SendHead()
{
	...
	USART_WriteDatas(USART2,"KQJHXLH000000001",16);
	DeBug("KQJHXLH000000001",4);
}
uint8 Connect=0;
void Task_M26Transmission(void *Tags)
{
	uint8 sta,i,FailCount=0,RFailCount;
//	HistoryRecord HR;
	Tos_ArrayMessage *CurMsg,Msg[3];
...
R:
	...
	UIShowValue.RSSI=99;
	sta=QuectelM26_Init();
	if(sta){DeBug("注册网络失败[%d] ",sta,Infor_Warning);GPIO_Set_PWR(0);if(UIShowValue.SaveValue.SleepMin){Task_RecordNow();TOS_Sleep(UIShowValue.SaveValue.SleepMin);};goto R;}
	else Task_AutoReg(0);
	sta=QuectelM26_InitSMS(1);
	if(sta)DeBug("初始化短信[%d]",sta,Infor_Warning);
C:
	sta=QuectelM26_Transmission();
	if(sta)goto R;
	Tos_TaskDelay(2000);
	SendHead();
	Connect=1;
	while(1)
	{
		i=USART_ReadDatas(USART2,Null,100,1000);
		if(i)
		{
			USART_RX_Buffer[1][i]=0;
			DeBug("%s",USART_RX_Buffer[1],Infor_Warning);
			if(strstr(USART_RX_Buffer[1],"CLOSED"))
			{
				Connect=0;
				goto R;
			}
			if(strstr(USART_RX_Buffer[1],"ERROR"))
			{
				Connect==0;
				goto R;
			}
			if(strstr(USART_RX_Buffer[1],"CONNECT FAIL"))
			{
				Connect==0;
				goto R;
			}
...
void UsartToGPRS(void *Tags)
{
	uint8 i;
	while(1)
	{
		Tos_TaskDelay(1);
		i=USART_ReadDatas(USART1,Null,100,1000);
		if(i)
		{
			USART_RX_Buffer[0][i]=0;
			if(Connect)
			{
				USART_RX_Buffer[0][i]=0;
				USART_WriteDatas(USART2,USART_RX_Buffer[0],i);
				DeBug("数据%s,长度=%d",USART_RX_Buffer[0],i,0);
			}
			else DeBug("Waiting",5);
			if(strstr(USART_RX_Buffer[0],"RESET"))
			{
				MCU_Reset();
			}
		}
