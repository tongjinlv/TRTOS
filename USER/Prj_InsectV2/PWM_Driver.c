...
void Pwm_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_AHBPeriph_GPIOB ,ENABLE);   /*打开APB2总线上的GPIOA时钟*/
	RCC_APB2PeriphClockCmd(RCC_AHBPeriph_GPIOC ,ENABLE);   /*打开APB2总线上的GPIOA时钟*/
	RCC_APB2PeriphClockCmd(RCC_AHBPeriph_GPIOA ,ENABLE);   /*打开APB2总线上的GPIOA时钟*/
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource6, GPIO_AF_TIM4);//L系列的比F系列的就多了个这
...
void PWM_Set(unsigned char PwmChannel,uint8  PwmValue)
{
	uint32 t;
	t=PWM_PERIOD;
	t= t*PwmValue/100;
