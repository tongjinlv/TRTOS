...
http://www.trtos.com/
**************************************************************************************/
#include <..\USER\Prj_TP500\DHT11_Driver.h>
...
uint8 DHT11_Read(float *Temp,float *Humi)
{
	...
	DHT11_Rst();
	if(DHT11_Check()){DeBug("检测失败",Infor_Error);return 0;}
	for(i=0;i<5;i++)
	{
		Data[i]=DHT11_Read_Byte();
	}
	c=Data[0]+Data[1]+Data[2]+Data[3];
	if(Data[4]==c)DeBug("校验成功",Infor_Infor);
	else {DeBug("校验失败",Infor_Error);return 0;}
