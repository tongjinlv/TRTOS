...
typedef  struct  usb_setup_req {
    ...
    uint8_t   bmRequest;    // 数据方向、请求类型、接受者                  
...
{
  uint8_t  *(*GetDeviceDescriptor)( uint8_t speed , uint16_t *length);  	 // This callback returns the device descriptor
  uint8_t  *(*GetLangIDStrDescriptor)( uint8_t speed , uint16_t *length); 	// This callback returns the Language ID string descriptor
