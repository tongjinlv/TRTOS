...
/**
  ...
  * @version V2.1.0
  * @date    19-March-2012
  * @brief   Peripheral Device interrupt subroutines	 Device模式下中断子程序集
  ******************************************************************************
  * @attention
  *
...
*         handles all USB Interrupts  处理所有USB中断
* @param  pdev: device instance
* @retval status
*/
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsDeviceMode(pdev)) /* ensure that we are in device mode */
  {
    ...
    if (gintr_status.b.outepintr)	// 输出端点中断
    {
      retval |= DCD_HandleOutEP_ISR(pdev);
    }    
    
    if (gintr_status.b.inepint)	  // 输入端点中断
    {
      retval |= DCD_HandleInEP_ISR(pdev);
    }
    
    if (gintr_status.b.modemismatch)   // 调制匹配
    {
    ...
    if (gintr_status.b.wkupintr)	   // 唤醒
    {
      retval |= DCD_HandleResume_ISR(pdev);
    }
    
    if (gintr_status.b.usbsuspend)	  // 挂起
    {
...
/**
* @brief  DCD_HandleInEP_ISR	输入端点中断处理
...
/**
* @brief  DCD_HandleOutEP_ISR	输出端点中断处理
...
static uint32_t DCD_HandleOutEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1)
    {
      ...
      /* Transfer complete 传送完成 */
      if ( doepint.b.xfercompl )
      {
        /* Clear the bit in DOEPINTn for this interrupt 清中断标志 */
